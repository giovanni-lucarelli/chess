...
def load_positions(csv_path: str = '../syzygy-tables/krk_dtz.csv'):
    """
...
def get_available_dtz_values() -> List[int]:
    """
    Get all available DTZ values in the dataset.
    
    Returns:
        List of unique DTZ values sorted in ascending order
    """
    positions, dtz_groups = load_positions()
    return sorted(dtz_groups.keys())

def get_dtz_count(dtz: int) -> int:
    """
    Get the number of positions available for a specific DTZ value.
    
    Args:
        dtz: The DTZ value to count
        
    Returns:
        Number of positions with the specified DTZ value
    """
    positions, dtz_groups = load_positions()
    return len(dtz_groups.get(dtz, []))
...
def get_stats() -> Dict[str, Union[int, Dict[int, int]]]:
    """
...
if __name__ == "__main__":
    ...
    try:
        positions = sample_endgames({1: 10, 3: 5, 11: 5})
