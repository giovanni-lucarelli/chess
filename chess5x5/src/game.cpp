...
Game::Game() {
    ...
    // Game state
    side_to_move = WHITE;
    // No castling in our 5x5 endgame
...
std::vector<Move> Game::pseudo_legal_moves(Square from) const {
    ...
    // No castling in our 5x5 endgame setup
    ...
    for (int to = 0; to < 25; ++to) {
        ...
        switch (p) {
            case PAWN:
                // No pawns in our endgame setup
                break;
                break;

            case KNIGHT:
                // Knights can still move in L-shape on 5x5 board
...
void Game::do_move(Move& move) {
    ...
    // No pawn logic in our endgame setup

    // No castling in our endgame setup

    // No castling rights to update in our endgame setup
...
void Game::undo_move(Move& move) {
    ...
    // No en passant in our endgame setup
    {
        ...
        // No promotions or castling to undo in our endgame setup
    }
...
Move Game::parse_move(const std::string& from, const std::string& to) const {
    Square from_sq = static_cast<Square>(5 * (from[1] - '1') + (from[0] - 'a'));
    Square to_sq = static_cast<Square>(5 * (to[1] - '1') + (to[0] - 'a'));
...
void Game::play() {
    ...
    while (true) {
        ...
        Square from = static_cast<Square>(5 * (str_from[1] - '1') + (str_from[0] - 'a'));
