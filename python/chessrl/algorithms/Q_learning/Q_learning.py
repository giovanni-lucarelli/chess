...
class QLearning_TDControl():
    def __init__(self, type_of_endgame, n_episodes = 2000, max_steps = 50, gamma=0.9, lr_v=0.1, epsilon = 0.15, save_path = "output/q_learning_policy.pkl"):
    ...
    def single_step_update(self, state, action, reward, new_state, new_legal_actions, done):
        ...
        for move in new_legal_actions:
            uci_move = chess_py.Move.to_uci(move)
            idx = self.positions_actions_to_idx[(new_state, uci_move)]
            q_val = self.Qvalues[idx]

            if q_val > best_value:
                best_value = q_val
                best_next_action = uci_move  # Exploit
        
        td_target = reward + (0 if done else self.gamma * self.Qvalues[self.positions_actions_to_idx[(new_state, best_next_action)]])
    ...
    def train(self):
        ...
        random_indices = np.random.choice(len(self.states), size=self.n_episodes, replace=False)
