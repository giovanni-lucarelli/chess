...
class ValueIterationProbabilistic:
    ...
    def train(self):
        ...
        for i in range(self.n_iterations):
            ...
            save_policy_jsonl(newPolicy, f"../../../../artifacts/policies/vi_probabilistic_{self.endgame_type}_greedy_intermediate_{i+1}.jsonl")
            save_values(states, newValues, f"../../../../artifacts/values/vi_probabilistic_{self.endgame_type}_greedy_intermediate_{i+1}.parquet")

        logger.info('Training completed.')
        
        # newPolicy : Dict[fen, uci]  |  states : List[fen]  |  newValues : np.ndarray
        save_policy_jsonl(newPolicy, f"../../../../artifacts/policies/vi_probabilistic_{self.endgame_type}_greedy.jsonl")
        save_values(states, newValues, f"../../../../artifacts/values/vi_probabilistic_{self.endgame_type}_values.parquet")
